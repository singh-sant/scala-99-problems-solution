...
object P11 extends App {
  ...
  def runLengthEncoding[T](list: List[List[T]]): List[Any] = {

    @tailrec
    def helper(list: List[List[T]], result: List[Any]): List[Any] = list match {
      case ::(head, tail) if (head.length == 1) => helper(tail, result :+ head.head)
